...
const { findUserByEmail, createUser, checkRateLimit } = require('../../_db');
...
module.exports = async (req, res) => {
  ...
  const allowed = await checkRateLimit(`register_${ip}`, 3, 3600);
  if (!allowed)
  ...
  try {
    ...
    await createUser({ email, password_hash, otp_code: otp, otp_expires_at });
